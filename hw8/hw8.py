...
class Person(object):
    ...
    def getFriends(self):
        return None if len(self.friends)==0 else self.friends
...
class Bird(object):
    ...
    def __repr__(self):
        return ((f'{self.species} has {self.eggs} egg') if self.eggs == 1 
            else (f'{self.species} has {self.eggs} eggs'))
    ...
    def __eq__(self, bird):
        return isinstance(bird, Bird ) and bird.species == self.species
